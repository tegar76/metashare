				...
				<form action="<?= $this->uri->uri_string() ?>" method="post">
					...
					<div class="mt-4 w-full">
						<div class="relative flex items-center justify-center">
							<input id="myInput" name="password" type="password" class="bg-gray-50 rounded-md text-xs font-medium leading-none placeholder-gray-800 text-gray-800 py-3 w-full pl-3 mt-2 p-2 border-b-2 border-b-gray-400 border-gray-300 outline-none opacity-80 focus:border-primary-blue-cyan/10" placeholder="Masukan Password" value="<?= set_value('password') ?>" required />
					...
					<div class="flex items-center justify-between mt-4">
						<div class="flex items-start">
							<div class="flex items-center h-5">
								<input id="remember" aria-describedby="remember" type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-0 checked:bg-success checked:hover:bg-success checked:focus:bg-success cursor-pointer">
							</div>
					...
					<div class="mt-6">
						<button type="submit" role="button" class="focus:ring-2 focus:ring-offset-2 focus:ring-primary text-sm font-semibold leading-none text-white focus:outline-none bg-primary-blue-cyan border rounded-md hover:bg-primary-blue-cyan-hover py-3 justify-center flex w-full transition duration-500">Login</button>
					</div>
				</form>
				<p tabindex="0" class="focus:outline-none text-sm mt-4 font-medium leading-none text-gray-500">Belum memiliki akun? <a href="<?= base_url('register') ?>" class="cursor-pointer hover:text-primary-blue-cyan-hover focus:text-primary-blue-cyan focus:outline-none focus:underline hover:underline text-sm font-medium leading-none text-primary-blue-cyan"> Sign Up</a></p>
			</div>
		</div>
