...
window.addEventListener("load",function(){
  setTimeout(function open(event){
       ...
       document.querySelector(".sampul").classList.add('animate__animated','animate__fadeInTopLeft','animate__delay-3.6s');
   },1000
...
document.querySelector("#buka").addEventListener("click",function(){
 setTimeout(function close(event){
     ...
     document.querySelector(".sampul").classList.add('animate__animated','animate__fadeOutTopRight','animate__delay-3.6s');
  },1000
  )  
  }
);
...
var countDownDate = new Date("Mar 28, 2023 13:43:00").getTime();
...
function setmus(){
 if (mc.paused) {
    mc.play();
    pm.src =  BASEURL + 'assets/navisa-racy/icon/in in-play.svg';    
 }else{
   mc.pause();
   pm.src = BASEURL + 'assets/navisa-racy/icon/in in-pause.svg';
 }
};
