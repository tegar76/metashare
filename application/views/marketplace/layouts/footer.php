...
<footer class="text-base-1xs xl:text-xs xl:text-base-xs <?= ($this->uri->segment(2) == "profile") || ($this->uri->segment(2) == "history") ? 'hidden' : '' ?>">
	<div class=" px-5 xl:px-20 pt-4 pb-2 xl:pt-5 xl:pb-5 bg-[#3B4B62] dark:bg-gray-900">
		<div class="">
			...
			<div class="sm:mx-12 md:mx-40 lg:mx-52 xl:mx-72 md:text-center lg:text-base-xs xl:text-sm leading-4 lg:leading-5 mt-3 text-slate-300 text-justify xl:text-center">Metashare adalah sebuah platfrom yang menyediakan berbagai macam model undangan Pernikahan digital berdasarkan Kategori kategori Special, Standard, dan Basic</div>
		...
		<div class="hidden xl:flex mt-4 text-sm space-x-6 xl:justify-center xl:mt-0">
			...
			<ul class="navbar-nav flex">
				<li class="nav-item p-2">
					<a class="nav-link text-slate-400 focus:text-primary-blue-cyan/90 hover:text-primary-blue-cyan/90 p-0 <?= ($this->uri->segment(2) == "") ? 'text-primary-blue-cyan font-semibold' : '' ?>" href="<?= base_url() ?>">Home</a>
				</li>
				<li class="nav-item p-2">
					<button class="nav-link text-slate-400 focus:text-primary-blue-cyan/90 hover:text-primary-blue-cyan/90 p-0 <?= ($this->uri->segment(2) == "categories") ? 'text-primary-blue-cyan font-semibold' : '' ?>" href="<?= base_url('categories') ?>" id="multiLevelDropdownButtonFoot" data-dropdown-toggle="dropdownFoot">Kategori <span><i class="fa fa-chevron-down fa-xs"></i></span></button>
					<!-- Dropdown menu -->
				...
				<li class="nav-item p-2">
					<a class="nav-link  text-slate-400 focus:text-primary-blue-cyan/90 hover:text-primary-blue-cyan/90 p-0 <?= ($this->uri->segment(2) == "testimony") ? 'text-primary-blue-cyan font-semibold' : '' ?>" href="<?= base_url('testimony') ?>">Testimoni</a>
				</li>
				<li class="nav-item p-2">
					<a class="nav-link  text-slate-400 focus:text-primary-blue-cyan/90 hover:text-primary-blue-cyan/90 p-0 <?= ($this->uri->segment(2) == "about") ? 'text-primary-blue-cyan font-semibold' : '' ?>" href="<?= base_url('about') ?>">Tentang</a>
				</li>
				<?php if ($this->session->userdata('logged_in') == true and $this->session->userdata('level') == 'customer') : ?>
					<!-- TRUE -->
					<li class="nav-item p-2">
						<a class="nav-link  text-slate-400 focus:text-primary-blue-cyan/90 hover:text-primary-blue-cyan/90 p-0 <?= ($this->uri->segment(2) == "profile") ? 'text-primary-blue-cyan font-semibold' : '' ?>" href="<?= base_url('profile') ?>">Profile</a>
					</li>
					<li class="nav-item p-2">
						<a class="logout-button nav-link  text-slate-400 focus:text-primary-blue-cyan/90 hover:text-primary-blue-cyan/90 p-0" href="#">Logout</a>
					</li>
				<?php else : ?>
					<!-- FALSE -->
					<li class="nav-item p-2">
						<a class="nav-link border bg-slate-600 rounded-xl border-slate-300 text-slate-300 hover:bg-primary-blue-cyan hover:text-white focus:bg-primary-blue-cyan focus:text-white py-1" href="<?= base_url('register') ?>"><span class="px-2">Sign Up</span></a>
					</li>
				<?php endif ?>
